...
var _ = Describe("EventsReporter", func() {
	...
	When("the app starts successfully", func() {
		...
		When("the app exists with non-zero code", func() {
			BeforeEach(func() {
				http.Get(fmt.Sprintf("http://%s.%s:8888/exit?exitCode=1", appServiceName, fixture.Namespace))
			})
		...
		When("the app exists with zero code", func() {
			BeforeEach(func() {
				http.Get(fmt.Sprintf("http://%s.%s:8888/exit?exitCode=0", appServiceName, fixture.Namespace))
			})
